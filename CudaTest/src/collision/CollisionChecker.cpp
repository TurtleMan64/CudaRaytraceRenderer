...
//Vector3d CollisionChecker::collidePosition;
	...
	float px2, float py2, float pz2)
{
	...
	float firstAbove = -2.0f;
	float secondAbove = -2.0f;
	float A, B, C, D;
    ...
    Triangle3D* collideTriangle = nullptr;
    ...
    return CollisionResult(triangleCollide, &collidePosition, collideTriangle);
}
...
bool CollisionChecker::checkPointInTriangle3D(
	...
	Triangle3D* tri)
{
    float nX = fabs(tri->normal.x);
    float nY = fabs(tri->normal.y);
    float nZ = fabs(tri->normal.z);
...
//}
//
//Vector3d* CollisionChecker::getCollidePosition()
//{
